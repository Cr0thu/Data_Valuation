...
with open("output_delete.txt", "a") as file:
...
for noise_level in noise_levels:
    ...
    for _ in range(10):
        ...
        score_change.append((np.array(mimic_label_delete_score)-np.array(pre_score)).mean())
        loss_change.append((np.array(mimic_label_delete_loss)-np.array(pre_loss)).mean())
        acc_change.append((np.array(mimic_label_delete_acc)-np.array(pre_acc)).mean())
    mimic_label_delete_score_mean = np.array(score_change).mean()
    mimic_label_delete_score_std = np.array(score_change).std()
    mimic_label_delete_loss_mean = np.array(loss_change).mean()
    mimic_label_delete_loss_std = np.array(loss_change).std()
    mimic_label_delete_acc_mean = np.array(acc_change).mean()
    mimic_label_delete_acc_std = np.array(acc_change).std()
    ...
    with open("output_delete.txt", "a") as file:
        file.write("{} & {}\\% & Delete & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} \\\\\n".format(
            penalty, noise_level, mimic_label_delete_score_mean, mimic_label_delete_score_std, mimic_label_delete_loss_mean, mimic_label_delete_loss_std, mimic_label_delete_acc_mean, mimic_label_delete_acc_std
        ))
