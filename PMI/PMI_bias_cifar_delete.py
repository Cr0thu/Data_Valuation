...
output_file = f"output_delete_{args.run_id}_train_{args.train_size_a_0}_{args.train_size_a_1}_{args.train_size_b_0}_{args.train_size_b_1}_test_{args.test_size_a_0}_{args.test_size_a_1}_{args.test_size_b_0}_{args.test_size_b_1}_penalty_{penalty_str_for_filename}_noise_{args.noise_level}_D_{args.D}.txt"
...
pre_score = []
mimic_label_delete_score = []
pre_loss = []
mimic_label_delete_loss = []
pre_acc = []
mimic_label_delete_acc = []
...
for d in tqdm(range(D), desc=f"GPU {args.gpu_id}, run_id {args.run_id}"):
    ...
    if d == 0:
        ...
        for i in range(num_candidate):
            ...
            print(f"  Mimic label delete model accuracy: {mimic_label_delete_train_data[i].base_acc:.4f}")
            print(f"  Mimic label delete model PMI score: {mimic_label_delete_train_data[i].score:.6f}")
            print(f"  Accuracy difference: {mimic_label_delete_train_data[i].base_acc - train_data[i].base_acc:.4f}")
            print(f"  PMI score difference: {mimic_label_delete_train_data[i].score - train_data[i].score:.6f}")
        print("---")

    for i in range(num_candidate):
        pre_score.append(train_data[i].score)
        mimic_label_delete_score.append(mimic_label_delete_train_data[i].score)
        pre_loss.append(train_data[i].base_loss)
        mimic_label_delete_loss.append(mimic_label_delete_train_data[i].base_loss)
        pre_acc.append(train_data[i].base_acc)
        mimic_label_delete_acc.append(mimic_label_delete_train_data[i].base_acc)
    
    # Periodically output PMI values for monitoring
    if d == 0:
        current_pre_score = torch.tensor(pre_score[-num_candidate:], device=device)
        current_mimic_score = torch.tensor(mimic_label_delete_score[-num_candidate:], device=device)
...
mimic_label_delete_score_tensor = torch.tensor(mimic_label_delete_score, device=device)
pre_loss_tensor = torch.tensor(pre_loss, device=device)
mimic_label_delete_loss_tensor = torch.tensor(mimic_label_delete_loss, device=device)
pre_acc_tensor = torch.tensor(pre_acc, device=device)
mimic_label_delete_acc_tensor = torch.tensor(mimic_label_delete_acc, device=device)

mimic_label_delete_score_mean = (mimic_label_delete_score_tensor - pre_score_tensor).mean().item()
mimic_label_delete_score_std = (mimic_label_delete_score_tensor - pre_score_tensor).std().item()
mimic_label_delete_loss_mean = (mimic_label_delete_loss_tensor - pre_loss_tensor).mean().item()
mimic_label_delete_loss_std = (mimic_label_delete_loss_tensor - pre_loss_tensor).std().item()
mimic_label_delete_acc_mean = (mimic_label_delete_acc_tensor - pre_acc_tensor).mean().item()
mimic_label_delete_acc_std = (mimic_label_delete_acc_tensor - pre_acc_tensor).std().item()
...
print(f"Average mimic label delete model accuracy: {mimic_label_delete_acc_tensor.mean().item():.4f} ± {mimic_label_delete_acc_tensor.std().item():.4f}")
print(f"Average mimic label delete model PMI score: {mimic_label_delete_score_tensor.mean().item():.6f} ± {mimic_label_delete_score_tensor.std().item():.6f}")
print(f"Accuracy change (mean ± std): {mimic_label_delete_acc_mean:.4f} ± {mimic_label_delete_acc_std:.4f}")
print(f"PMI score change (mean ± std): {mimic_label_delete_score_mean:.6f} ± {mimic_label_delete_score_std:.6f}")
print(f"Loss change (mean ± std): {mimic_label_delete_loss_mean:.4f} ± {mimic_label_delete_loss_std:.4f}")
print("=" * 50)
...
with open(output_file, "a") as file:
    try:
        ...
        file.write("{} & Delete & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f}\n".format(
            penalty, mimic_label_delete_score_mean, mimic_label_delete_score_std, 
            mimic_label_delete_loss_mean, mimic_label_delete_loss_std, 
            mimic_label_delete_acc_mean, mimic_label_delete_acc_std
        ))
        file.flush()  # 确保数据立即写入
