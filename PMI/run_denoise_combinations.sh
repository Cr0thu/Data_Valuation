...
TEST_CONFIGS=("20 60 60 20" "200 600 600 200")
PENALTIES=(0.1 1 100 1000)
NOISE_LEVELS=(10)
...
run_single_experiment() {
    ...
    if ./run_denoise_parallel.sh \
        ...
        # 复制结果文件
        if [ -f "output_denoise_final.txt" ]; then
            cp "output_denoise_final.txt" "$result_file"
...
generate_summary() {
    ...
    # 只删除单个实验输出，不删除 output_denoise_final.txt
    rm -f output_denoise_[0-9]*.txt 2>/dev/null || true
