...
run_single_experiment() {
    ...
    if ./run_delete_parallel.sh \
        ...
        # 复制结果文件
        if [ -f "output_delete_final.txt" ]; then
            cp "output_delete_final.txt" "$result_file"
...
generate_summary() {
    ...
    # 只删除单个实验输出，不删除 output_delete_final.txt
    rm -f output_delete_[0-9]*.txt 2>/dev/null || true
