...
output_file = f"output_denoise_{args.run_id}_train_{args.train_size_a_0}_{args.train_size_a_1}_{args.train_size_b_0}_{args.train_size_b_1}_test_{args.test_size_a_0}_{args.test_size_a_1}_{args.test_size_b_0}_{args.test_size_b_1}_penalty_{penalty_str_for_filename}_noise_{args.noise_level}_D_{args.D}.txt"
...
def data_denoise(train_data, num_candidate, ratio = 0.5):
    ...
    for i in range(num_candidate):
        ...
        new_train_data.append(dataset(new_train_X, new_train_y, new_train_X.size()[0], 0, 0, 0, train_data[i].noise*(1-ratio), bias_tensor, [], []))
...
for d in tqdm(range(D), desc=f"GPU {args.gpu_id}, run_id {args.run_id}"):
    ...
    if d == 0:
        ...
        for i in range(num_candidate):
            ...
            print(f"  Data denoise model accuracy: {data_denoise_train_data[i].base_acc:.4f}")
            print(f"  Data denoise model PMI score: {data_denoise_train_data[i].score:.6f}")
            print(f"  Accuracy difference: {data_denoise_train_data[i].base_acc - train_data[i].base_acc:.4f}")
            print(f"  PMI score difference: {data_denoise_train_data[i].score - train_data[i].score:.6f}")
        print("---")

    for i in range(num_candidate):
        pre_score.append(train_data[i].score)
        data_denoise_score.append(data_denoise_train_data[i].score)
        pre_loss.append(train_data[i].base_loss)
        data_denoise_loss.append(data_denoise_train_data[i].base_loss)
        pre_acc.append(train_data[i].base_acc)
        data_denoise_acc.append(data_denoise_train_data[i].base_acc)
    
    # Periodically output PMI values for monitoring
    if d == 0:
        current_pre_score = torch.tensor(pre_score[-num_candidate:], device=device)
        current_mimic_score = torch.tensor(data_denoise_score[-num_candidate:], device=device)
...
data_denoise_score_tensor = torch.tensor(data_denoise_score, device=device)
pre_loss_tensor = torch.tensor(pre_loss, device=device)
data_denoise_loss_tensor = torch.tensor(data_denoise_loss, device=device)
pre_acc_tensor = torch.tensor(pre_acc, device=device)
data_denoise_acc_tensor = torch.tensor(data_denoise_acc, device=device)

data_denoise_score_mean = (data_denoise_score_tensor - pre_score_tensor).mean().item()
data_denoise_score_std = (data_denoise_score_tensor - pre_score_tensor).std().item()
data_denoise_loss_mean = (data_denoise_loss_tensor - pre_loss_tensor).mean().item()
data_denoise_loss_std = (data_denoise_loss_tensor - pre_loss_tensor).std().item()
data_denoise_acc_mean = (data_denoise_acc_tensor - pre_acc_tensor).mean().item()
data_denoise_acc_std = (data_denoise_acc_tensor - pre_acc_tensor).std().item()
...
print(f"Average data denoise model accuracy: {data_denoise_acc_tensor.mean().item():.4f} ± {data_denoise_acc_tensor.std().item():.4f}")
print(f"Average data denoise model PMI score: {data_denoise_score_tensor.mean().item():.6f} ± {data_denoise_score_tensor.std().item():.6f}")
print(f"Accuracy change (mean ± std): {data_denoise_acc_mean:.4f} ± {data_denoise_acc_std:.4f}")
print(f"PMI score change (mean ± std): {data_denoise_score_mean:.6f} ± {data_denoise_score_std:.6f}")
print(f"Loss change (mean ± std): {data_denoise_loss_mean:.4f} ± {data_denoise_loss_std:.4f}")
print("=" * 50)
...
with open(output_file, "a") as file:
    try:
        ...
        file.write("{} & Denoise & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f}\n".format(
            penalty, data_denoise_score_mean, data_denoise_score_std, 
            data_denoise_loss_mean, data_denoise_loss_std, 
            data_denoise_acc_mean, data_denoise_acc_std
        ))
        file.flush()  # 确保数据立即写入
