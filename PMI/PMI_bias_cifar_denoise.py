...
with open("output_denoise.txt", "a") as file:
...
for noise_level in noise_levels:
    ...
    for _ in range(10):
        ...
        score_change.append((np.array(data_denoise_score)-np.array(pre_score)).mean())
        loss_change.append((np.array(data_denoise_loss)-np.array(pre_loss)).mean())
        acc_change.append((np.array(data_denoise_acc)-np.array(pre_acc)).mean())
    data_denoise_score_mean = np.array(score_change).mean()
    data_denoise_score_std = np.array(score_change).std()
    data_denoise_loss_mean = np.array(loss_change).mean()
    data_denoise_loss_std = np.array(loss_change).std()
    data_denoise_acc_mean = np.array(acc_change).mean()
    data_denoise_acc_std = np.array(acc_change).std()
    ...
    with open("output_denoise.txt", "a") as file:
        file.write("{} & {}\\% & Denoise & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} & {:.4f}\\pm{:.4f} \\\\\n".format(
            penalty, noise_level, data_denoise_score_mean, data_denoise_score_std, data_denoise_loss_mean, data_denoise_loss_std, data_denoise_acc_mean, data_denoise_acc_std
        ))
