...
run_instance() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        if wait_for_resources $GPU_ID; then
            # Start the process
            CUDA_VISIBLE_DEVICES=$GPU_ID python PMI_bias_cifar_delete.py $COMMON_ARGS --gpu_id 0 --run_id $RUN_ID &
            local pid=$!
...
output_files = glob.glob(f"output_delete_*_train_{train_size_a_0}_{train_size_a_1}_{train_size_b_0}_{train_size_b_1}_test_{test_size_a_0}_{test_size_a_1}_{test_size_b_0}_{test_size_b_1}_penalty_{penalty_str_for_filename}_noise_{noise}_D_{d_value}.txt")
...
if all_scores:
    ...
    with open('output_delete_final.txt', 'a+') as f:
        ...
        f.write(f"{penalty} & {noise} & Delete & {score_mean:.4f}\\\\pm{score_std:.4f} & {loss_mean:.4f}\\\\pm{loss_std:.4f} & {acc_mean:.4f}\\\\pm{acc_std:.4f}\n")
        f.flush()  # 确保数据立即写入

    print("Results have been combined and written to output_delete_final.txt")
else:
